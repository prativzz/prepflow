    ...
    <>
      <header className="h-16 w-full flex items-center justify-between px-6 border-b border-neutral/20 bg-white/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <nav className="flex items-center gap-4">
          ...
          {isAuthenticated ? (
            <Link to="/dashboard" className="px-4 py-2 text-sm font-medium bg-primary text-white rounded-md hover:bg-primary-dark transition-all">Go to Dashboard</Link>
          ) : (
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 text-center mt-20">
        <div className="max-w-2xl space-y-6">
          ...
          <div className="pt-4 flex gap-4 justify-center">
            {isAuthenticated ? (
              <Link to="/dashboard" className="px-8 py-3 bg-primary text-white font-semibold rounded-lg hover:bg-primary-dark hover:-translate-y-0.5 transition-all shadow-md">
                Go to Dashboard
              </Link>
            ) : (
